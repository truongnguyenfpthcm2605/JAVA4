...
<c:url var="app"  value="/bai3"></c:url>
	<h2>Danh sách Yêu Thích Video Theo Tiêu Đề</h2>
	<hr>
	<form action="${app }/findByKeyword" method="post"
	...
	<table class="table table-striped"
		...
		<tbody>
			<c:choose>
				<c:when test="${not empty videoToTitle }">
					<c:forEach var="item" items="${videoToTitle}">
						<tr>
							<td>${item.id }</td>
							<td>${item.title }</td>
							<td>${item.poster }</td>
